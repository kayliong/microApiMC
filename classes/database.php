...
class Database 
{
    ...
    */
	...
	public function update($updateValue, $where, &$result = array(), $onlySql = false) {
	    ...
	    $this->_sendQuery($sql, $params, 'master', $result);print_r($sql); print_r($params);
	    $this->debugResult($result, 'db_affected_num');
	    if (is_int($result['affected_num']) && $result['affected_num'] >= 0) {
	        return true;
	    }
	...
	public function connect($masterOrSlave, $reConnect){
	    ...
	    $appName = ENVIRONMENT;
	    $config = Core_Conf::getConf('database/' . ENVIRONMENT);
        ...
        if(empty($username)){
            $username = Helper_Utils::getValue($config, array($masterOrSlave, 'user'));
        }
        if(empty($password)){
            $password = Helper_Utils::getValue($config, array($masterOrSlave, 'pass'));
        }
        if(empty($hostspec)){
            $hostspec = Helper_Utils::getValue($config, array($masterOrSlave, 'host'));
        }
        if(empty($port)){
            $port =     Helper_Utils::getValue($config, array($masterOrSlave, 'port'));
        }
        if(empty($database)){
            $database = Helper_Utils::getValue($config, array($masterOrSlave, 'name'));
        }
